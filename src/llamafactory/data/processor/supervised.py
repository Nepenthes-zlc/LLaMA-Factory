...
class SupervisedDatasetProcessor(DatasetProcessor):
    def _encode_data_example(
        ...
        for turn_idx, (source_ids, target_ids) in enumerate(encoded_pairs):
            ...
            if self.data_args.mask_empty_controls and target_len > 0:
                target_text = self.tokenizer.decode(target_ids[:target_len], skip_special_tokens=False)
                empty_marker = "<available_controls>\n[]\n</available_controls>"
                marker_pos = target_text.find(empty_marker)
                if marker_pos != -1:
                    prefix_ids = self.tokenizer.encode(target_text[:marker_pos], add_special_tokens=False)
                    mask_start = len(prefix_ids)
                    target_label = target_label[:mask_start] + [IGNORE_INDEX] * (target_len - mask_start)
